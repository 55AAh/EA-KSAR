...
class NppTable(BaseTable):
    ...
    num: Mapped[int] = mapped_column(Numeric(1), comment="Номер АЕС")
    sh_name: Mapped[str] = mapped_column(String(10), comment="Позначення АЕС")
    name: Mapped[str] = mapped_column(String(50), comment="Найменування АЕС")
    descr: Mapped[str | None] = mapped_column(
        String(2000), nullable=True, comment="Опис АЕС"
    )
    sh_name_eng: Mapped[str] = mapped_column(
        String(10), comment="Позначення АЕС (англ.)"
    )
    name_eng: Mapped[str] = mapped_column(
        String(50), comment="Найменування АЕС (англ.)"
    )

    # Relationships
    units: Mapped[list["NppUnitTable"]] = relationship(back_populates="plant")
