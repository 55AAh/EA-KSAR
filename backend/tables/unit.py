...
class NppUnitTable(BaseTable):
    ...
    plant_id: Mapped[int] = mapped_column(
        ForeignKey("T_NPPS.plant_id"), comment="ID АЕС"
    )
    num: Mapped[int] = mapped_column(Numeric(1), comment="Номер блоку")
    name: Mapped[str] = mapped_column(String(30), comment="Найменування блоку")
    name_eng: Mapped[str] = mapped_column(
        String(30), comment="Найменування блоку (англ.)"
    )
    design: Mapped[str] = mapped_column(String(30), comment="Проект")
    stage: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="Черга"
    )
    power: Mapped[int] = mapped_column(
        Numeric(6, 2), comment="Встановлена потужність, МВт"
    )
    ...
    # Relationships
    plant: Mapped["NppTable"] = relationship(back_populates="units")
    reactor_vessel: Mapped["ReactorVesselTable"] = relationship(back_populates="unit")
