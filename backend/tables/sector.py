...
class ReactorVesselSectorTable(BaseTable):
    ...
    rpv_sector_id: Mapped[int] = mapped_column(
        Integer, Identity(), primary_key=True, comment="ID сектора"
    )
    vessel_id: Mapped[int] = mapped_column(
        ForeignKey("T_NPU_RCT_VESSELS.vessel_id"),
        comment="ID корпусу реактора",
    )
    sector_number: Mapped[int] = mapped_column(Integer, comment="№ сектору")

    # Relationships
    vessel: Mapped["ReactorVesselTable"] = relationship(back_populates="sectors")
    placements: Mapped[list["PlacementTable"]] = relationship(back_populates="sector")
