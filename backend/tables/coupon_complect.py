...
class CouponComplectTable(BaseTable):
    ...
    name: Mapped[str] = mapped_column(String(3), comment="Позначення комплекту")
    complect_number: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Порядковий номер комплекту"
    )
    ...
    container_systems: Mapped["ContainerSysTable"] = relationship(
