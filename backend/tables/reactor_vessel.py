...
class ReactorVesselTable(BaseTable):
    ...
    vessel_id: Mapped[int] = mapped_column(
        Integer, Identity(), primary_key=True, comment="ID корпусу реактора"
    )
    unit_id: Mapped[int] = mapped_column(
        ForeignKey("T_NPP_UNITS.unit_id"), comment="ID енергоблоку"
    )

    # Relationships
    unit: Mapped["NppUnitTable"] = relationship(back_populates=None)
